...
numpy の古いバージョンを試してください。動作確認済み(pyscf=2.11,numpy=1.26,2.0.0,2.1.0,2.2.0,2.3.0)
...
>>cat project.prm
AAP # 物質名
3,19,10 # 投影面を定義する原子の番号、また、頭の二つの原子間距離を計算する。
flip:False # 裏返す。
rotate:240.0 # 回転させる。
```

パラメータファイルを利用して投影図を作成します。標準出力に、結合のある原子間、及び、指定した原子間の距離が表示されます。
```
...
投影図を作成した時に、H(#3)とO(#19)の原子間距離が2.00Åと得られているので、この距離から単結合相当の1.00Åまで近づけていきます。
...
```
解析手順②で利用した設定ファイル project.prm を利用して、tdd ファイルより吸収スペクトル、txt ファイルより投影図を作成します。
...
```
>> cat plotEnergy.prm
files:../esipt/AAP,../esipt2/AAP # データのあるディレクトリーの指定
label:'O H','N-H' # 凡例のラベル
output:AAP_erg #　出力ファイルの頭文字
ops:[0.9,2.1],60 # グラフ化のパラメータ：横軸 0.9-2.1 Å、縦軸 0-60 kcal/mol
```
