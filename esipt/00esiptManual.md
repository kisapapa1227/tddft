...
- 投影図作成のためのパラメータファイル(例：project.prm)を準備します。
...
03-19 : 2.00Å O H
```
この処理は結合している原子対や指定原子間の距離を標準出力に表示します。
...
- 一連のtdd/txt から GIF を作成します:
```
>>sh ../bin/esiptTxt2Gif.sh project.prm
```
- project.prm (解析手順②で利用したもの)を用いて、.tdd から吸収スペクトル、.txt から投影図を作成し、最終的に AAP_esipt_cb.gif (例)が作成されます。
...
- プロット用のパラメタファイルの例 plotEnergy.prm:
```
...
### 二面角(ジヘドラル)指定による解析
- 一重結合の回転を模した解析（ジアヘドラル角を拘束）も可能です。-phase 3 を指定すると、基底状態エネルギーの最適化のみで計算を打ち切り、解析時間を短縮できます（このモードでは吸収スペクトルは生成されません）。
- 
例:
```
...
サンプル(dihedral)では C#2-C#9間の一重結合が回転した時のポテンシャルエネルギーの変化を評価しています。結果のプロットも esiptErgPlot.py を用いて作成します。
