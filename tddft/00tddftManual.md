...
```
1行目が処理をする tdd ファイルのリスト、二行目が出力ファイルの末尾名、三行目がグラフ化のパラメータになります。
この例では横軸が100-700nmで601等分(=1nm間隔)、縦軸の吸収強度が0-0.60 a.u.、共振強度から吸収スペクトルを作成する時の分散に関連するパラメータを4000 に設定します。分散に関連するパラメータが大きいと、ピーク分布の半値幅が小さくなります。補足説明 3.4 stddev の比較を参照してください。出力ファイル名は"ディレクトリー名_末尾名.pptx"になります。
必要に応じて、このファイルを書き換えます。
...
-N:吸収スペクトルを正規化（最大値で無次元化）し、最大値を凡例に書く。
-s:uv域（220-300nm）の最大値で正規化する。
-up 280: 閾値 threshold (280nm) 以上に吸収ピークの存在するの分布のみ描画する。
-r 0.1: 上の条件で、正規化した時のピーク高が threshold(10%) 以上の分布のみ描画する。
-num 5: グラフ毎（ページ毎）に描画する吸収スペクトルの数を指定する。 
-no_table: 表を描きません。
```

### 2.2 エクセルファイルで作成する。
次のコマンドでエクセルファイルのレポートが作成されます。
```
...
-n tag_name:データを書き込むシートのタグ名を指定する。デフォルトはデータのディレクトリー名です。
-k key: key をファイル名に含む tdd ファイルのみ処理する。
-range x0,x1,num: 横軸の最小値、最大値、分割数を指定する。デフォルトは100,600,501 です。
-stddev stddev:共振強度から吸収スペクトルを作成する時の分散に関連するパラメータを指定する。デフォルト値は4000です。 
-o outputFileName:出力するファイル名を指定する。デフォルトは、uv_all.xlsxです。
```
