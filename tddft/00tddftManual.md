...
-r 0.1: 上の条件に吸収ピークが存在しても、正規化した時のピーク高が threshold(10%) 以下の場合は除外します。
-num 5: 一グラフ（一ページ）に描画する吸収スペクトルの数を決めます。 
-no_table: 表を描きません。
```
...
-n tag_name:データを書き込むシートのタグ名を指定します。デフォルトはデータのディレクトリー名です。
-k key: key をファイル名に含む tdd ファイルのみ処理します。
